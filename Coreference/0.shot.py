...
class RteProcessor(DataProcessor):
    ...
    def get_GAP_coreference_as_test(self, filename):
        '''
        selected_examples: [(idd, premise, hypy_A, entity_A_label, hypy_B, entity_B_label)]
        '''
        examples=[]
        selected_examples = load_GAP_coreference_data(filename, 0)
...
def main():
    ...
    eval_all_input_indices = torch.tensor([f.id for f in test_features], dtype=torch.long)
    ...
    eval_data = TensorDataset(eval_all_input_indices, eval_all_input_ids, eval_all_input_mask, eval_all_segment_ids, eval_all_label_ids)
